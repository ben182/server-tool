...
cp ${TEMPLATES_PATH}apache/vhost.conf /etc/apache2/sites-available/$DOMAIN.conf
...
select yn in "Non SSL to SSL and www to non www" "www to non www" "Nothing"; do
    case $yn in
        ...
        cp ${TEMPLATES_PATH}apache/nonSSL_to_SSL_and_www_to_nonwww.htaccess /var/www/$DOMAIN/html/.htaccess
        break;;

        "www to non www" )
        cp ${TEMPLATES_PATH}apache/www_to_nonwww.htaccess /var/www/$DOMAIN/html/.htaccess
        break;;
